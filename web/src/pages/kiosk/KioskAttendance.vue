...
async function submitMarkPresent() {
  ...
  try {
    ...
    const { error: attError } = await supabase.rpc('toggle_contract_attendance', {
      p_tenant_id: kioskStore.tenantId,
      ...
      p_staff_id: kioskStore.currentStaff?.id,
    });
...
async function viewBreakdownById(customerId: string) {
  const cust = customersStore.customers.find((c) => c.id === customerId);
  if (cust) {
...
async function loadBreakdownData(customerId: string) {
  ...
  try {
    ...
    breakdownAttendance.value = (attRes.data ?? []) as AttendanceRecord[];
    breakdownBaki.value = (bakiRes.data ?? []) as BakiItem[];
    breakdownCollections.value = (colRes.data ?? []) as CollectionItem[];
  } catch (err) {
...
async function fetchData() {
  ...
  try {
    ...
    // 3. Fetch contract worker customers via store
    ...
    let attQuery = supabase.from('customer_daily_attendance').select('*').eq('tenant_id', tenantId);
    let bakiQuery = supabase.from('baki_transactions').select('*').eq('tenant_id', tenantId);
    let colQuery = supabase.from('customer_collections').select('*').eq('tenant_id', tenantId);
    ...
    if (attRes.error) throw attRes.error;
    attendanceList.value = (attRes.data ?? []) as AttendanceRecord[];

    if (activeTab.value === 'monthly') {
      if (bakiRes.error) throw bakiRes.error;
      if (colRes.error) throw colRes.error;
      bakiTransactions.value = (bakiRes.data ?? []) as BakiItem[];
      collectionsList.value = (colRes.data ?? []) as CollectionItem[];
    }
  } catch (err) {
