    ...
    >
      ...
      <q-card-section class="row items-center border-bottom q-py-sm">
        <div>
          <div class="text-subtitle1 text-weight-bold text-primary">
            {{ $t('customers.advancePayment.dialog.title') }}
          </div>
      ...
      <q-card-section class="q-pa-md">
        <q-form @submit.prevent="onSubmit" class="q-gutter-y-sm">
          ...
          <div class="column bg-grey-1 border-all rounded-borders q-pa-sm q-mb-xs">
            <div class="row justify-between items-center">
              <span class="text-caption text-weight-bold text-slate-700">
                {{ $t('customers.advancePayment.dialog.currentBalance') }}
              </span>
              <span class="font-mono text-subtitle2 text-weight-bold text-slate-800">
          ...
          <!-- Prepaid Hint/Caption -->
          <div class="text-caption text-grey-6 q-mb-sm leading-normal">
            {{ $t('customers.advancePayment.dialog.prepaidHint') }}
          </div>

          <!-- Amount Input -->
          <div>
            <label class="block text-caption text-grey-7 text-weight-medium q-mb-xs">
              {{ $t('customers.advancePayment.dialog.amount') }} *
            </label>
            <q-input
              ...
              :rules="[
                (val) =>
                  ...
                  $t('customers.advancePayment.dialog.amountRequired'),
                (val) => val > 0 || $t('customers.advancePayment.dialog.amountMin'),
              ]"
              :disable="saving"
              hide-bottom-space
          ...
          <div>
            <label class="block text-caption text-grey-7 text-weight-medium q-mb-xs">
              {{ $t('customers.advancePayment.dialog.paymentMethod') }} *
            </label>
            <q-btn-toggle
            ...
            >
              {{ $t('customers.advancePayment.dialog.sessionRequiredCash') }}
            </div>
          </div>
          ...
          <div>
            <label class="block text-caption text-grey-7 text-weight-medium q-mb-xs">
              {{ $t('customers.advancePayment.dialog.notes') }}
            </label>
            <q-input
              v-model="form.notes"
              dense
          ...
          <div class="row justify-end q-gutter-x-sm q-mt-lg border-top q-pt-md">
            <q-btn
              ...
              dense
              no-caps
              :label="$t('customers.advancePayment.dialog.cancelBtn')"
              v-close-popup
            ...
            <q-btn
              ...
              type="submit"
              color="primary"
              :label="$t('customers.advancePayment.dialog.saveBtn')"
              :loading="saving"
...
const isSaveDisabled = computed(() => {
  ...
  if (form.value.amount === null || form.value.amount <= 0) return true;
...
async function onSubmit() {
  ...
  try {
    ...
    feedback.showSuccess(t('customers.advancePayment.dialog.success'));
